...
GPXParser.prototype.AddTrackSegmentToMap = function(trackSegment, colour, width)
{
	...
	{
        // CDM Check to see if the trackpoint has a desc node
        if (trackpoints[i].getElementsByTagName("desc").length > 0) {
...
GPXParser.prototype.AddWaypointsToMap = function ()
{
    ...
    function onLoad(xmlSource)
    {
        MyMap = new GMap2(document.getElementById("map_canvas"));
        MyMap.addControl(new GLargeMapControl());
        MyMap.addControl(new GMapTypeControl());
        LoadGPXFileIntoGoogleMap(MyMap, xmlSource);
    }
